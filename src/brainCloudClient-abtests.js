...
function BCAbTest () {
  ...
  bc.abtests.getABTest = function (abTestingId, abTestName) {
    ...
    for (var x = 0; x < abTestData.ab_tests.length; x++) {
      if (
        abTestData.ab_tests[x].name == abTestName &&
        abTestData.ab_tests[x].active == 'true'
      ) {
          ...
          if (abTestingId >= minId && abTestingId <= maxId) {
            console.log(
              "getABTest() - Found AB test '" +
                abTestName +
                ':' +
                abTestData.ab_tests[x].data[y].name +
                "' for abTestingId '" +
                abTestingId +
                "' in range '" +
                minId +
  ...
  bc.abtests.pushABTestResult = function (
    abTestingId,
    abTestName,
    abSelection,
    result
